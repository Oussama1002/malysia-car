    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap items-start gap-3">
          ...
          <GeneratePdfButton kind="contract" entityId={String(c.id ?? id)} className="rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm font-bold text-slate-800 hover:bg-slate-50 transition-colors disabled:opacity-50" />
          {c.status === 'active' && (
            <button
              type="button"
              onClick={() => setShowSwapVehicle(true)}
              className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 hover:bg-slate-50 transition-colors"
            >
              Changer de véhicule
            </button>
          )}
