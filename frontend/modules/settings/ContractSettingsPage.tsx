...
export const ContractSettingsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-black text-slate-900 dark:text-white">Paramètres Contrats & Réservations</h1>
        <p className="text-slate-500 dark:text-slate-400">Valeurs par défaut, limites de durée et conditions financières.</p>
      </header>

      <div className="space-y-8">
        {contractSections.map(([section, fields]) => (
          <div key={section} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
            <h2 className="text-[15px] font-black text-slate-900 dark:text-white mb-4">{section}</h2>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {fields.map((f) => (
                <div key={f.key}>
                  <label className="mb-1 block text-xs font-bold text-slate-500 dark:text-slate-400">{f.label}</label>
                  <div className="flex items-center gap-2">
                    <input
                      type={f.type}
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-900 dark:text-white"
                      placeholder={f.placeholder}
                      value={contractValues[f.key] ?? ''}
                      onChange={(e) => setContractValues((v) => ({ ...v, [f.key]: e.target.value }))}
                    />
                    {f.suffix && <span className="text-xs font-bold text-slate-400 whitespace-nowrap">{f.suffix}</span>}
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={f.key}>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-900 dark:text-white"
                      placeholder={f.placeholder}
                      value={reservationValues[f.key] ?? ''}
                      onChange={(e) => setReservationValues((v) => ({ ...v, [f.key]: e.target.value }))}
                    />
                    {f.suffix && <span className="text-xs font-bold text-slate-400 whitespace-nowrap">{f.suffix}</span>}
                  </div>
                </div>
      ...
      <div className="flex items-center gap-4">
        <button
          type="button"
          className="rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-black text-white shadow-lg shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50"
